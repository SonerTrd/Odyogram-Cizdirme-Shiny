...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    tags$b(p("Yonerge")),
    p("Her bir frekans icin asagidaki bosluklari doldurun ya da yukari/asagi ok ile degerini giriniz. Her kutuya -10 ile 100 (ikisi de dahil) arasinda bir deger girebilirsiniz. Eger o frekansta bir deger yok ise bos birakiniz."),    
    p("Eger katilimciya ait isitme esiklerini normatif verilerle karsilastirmak isterseniz, 'Plot ISO percentiles' sekmesine tiklayiniz ve katilimciya ait yas ile cinsiyeti dogru giriniz. Normatif yuzdelik dilimlere ait veriler ISO 7029:2017 veriseti tablo D1'den alinmistir."),  
      "Yuzdelik degerler 80 yas ustu katilimcilar icin 3000 Hz ile 8000 Hz arasindaki degerler icin sadece bilgi amaclidir."
    ),
  ...
  mainPanel(
    plotOutput("odyogram", width = 800, height = 600)
      )
  
  ), 

    
  fluidRow(style = "padding: 1em 1em 2em", 
...
server <- function(input, output){
  ...
  plotInput <- reactive({
    ...
    geom_line(data = thresholds.R.final(), aes(x = freq, y = threshold), size = 0.6, linetype = "solid", colour = "red") + 
    geom_point(data = thresholds.R.final(), aes(x = freq, y = threshold), size = 5, stroke = 1.5, shape = 1, colour = "red") +
    geom_text(data = thresholds.R.final(), aes(label = "R", x = (freq[1] - 25), y = threshold[1]), color = "red", size = 5) + 
    # Plot left ear second
    geom_line(data = thresholds.L.final(), aes(x = freq, y = threshold), size = 0.6, linetype = "longdash", colour = "blue") + 
    geom_point(data = thresholds.L.final(), aes(x = freq, y = threshold), size = 5, stroke = 1.5, shape = 4, colour = "blue") +
  ...
  output$downloadPNG <- downloadHandler(
      filename = function() { 
        paste0("odyogram-", Sys.Date(), ".png", sep='') },
      content = function(file) { 
        png(file, width = 800, height = 600, res= NA)
        print(plotInput())
        dev.off()
